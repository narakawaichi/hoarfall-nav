    ...
    <div className="fixed bottom-0 left-0 w-full h-32 pointer-events-none z-10 overflow-hidden transition-colors duration-1000">
      ...
      {blades.map(blade => (
        <div key={blade.id} className="absolute bottom-0 origin-bottom flex items-end"
          ...
          <div
            // 白天变绿，晚上变白（只过渡颜色，避免 transition-all 在切换时重算全部属性）
            className={`w-full h-full transition-colors duration-300 ${isDark ? 'bg-gradient-to-t from-white/80 to-transparent' : 'bg-gradient-to-t from-emerald-500/80 to-transparent'}`}
